        ...
        public ActionResult Add(Korisnik korisnik, string ponovoLozinka, IFormFile ifile)
        {
            ...
            if (isEmail)
            {
                if (korisnik.TipKorisnika == Tip.POTROSAC)
                {
                    ...
                    Message message = new Message(new string[] { "djordjelazarevic1997@gmail.com" }, "[Web 2] - Projekat", "Uspesno ste se registrovali na sistem, mozete koristiti usluge naseg sistema!");
                ...
                else if (korisnik.TipKorisnika == Tip.ADMINISTRATOR)
                {
                    korisnik.Verifikovan = Zahtev.PRIHVACEN;
                    Message message = new Message(new string[] { "djordjelazarevic1997@gmail.com" }, "[Web 2] - Projekat", "Uspesno ste se registrovali kao administrator na sistem!");
                    _emailSender.SendEmail(message);

                    HttpContext.Session.SetString("AlertMessage", "Administrator uspesno registrovan!");
                    HttpContext.Session.SetString("Uspesno", JsonConvert.SerializeObject(true));
                }
                else
                {
                    korisnik.Verifikovan = Zahtev.PROCESIRA_SE;
                    Message message = new Message(new string[] { "djordjelazarevic1997@gmail.com" }, "[Web 2] - Projekat", "Vas nalog se procesuira, administrator ce obraditi vas zahtev u najkracem mogucem roku!");
                    _emailSender.SendEmail(message);
